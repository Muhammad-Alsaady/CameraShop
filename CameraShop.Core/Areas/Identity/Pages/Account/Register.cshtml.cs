...
namespace CameraShop.Core.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            {
                CompanyList = await unitOfWork.Companies.GetAllList(),
                RoleList =  roleManager.Roles.Select(r => new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem { Text = r.Name, Value = r.Id})
                //    Where(r => r.Name != SD.Role_User_Individual).Select(r => r.Name)
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                #region Assign role to users
                if (user.Role == SD.Role_Admin)
                    await _userManager.AddToRoleAsync(user, SD.Role_Admin);
                else
                {
                    if (user.CompanyId > 0 ) /// User has a Company Role
                    {
                        await _userManager.AddToRoleAsync(user, SD.Role_User_Company);
                    }
                    if (user.Role is null || user.CompanyId is null)
                ...
                if (result.Succeeded)
                {
                    ...
                    else
                    {
                        ...
                        else return RedirectToAction(nameof(Index), "User", new {Area = "Admin"});
                    }
                }
